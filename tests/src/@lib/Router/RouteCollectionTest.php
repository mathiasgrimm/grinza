...
class RouteCollectionTest extends TestCase
{
    ...
    public function validate_works_when_any_required_attribute_is_missing()
    {
        ...
        $this->assertEquals('Route does not have controller and therefore cant be added', $e->getMessage());

        try {
            $routes->addRoute(new Route(null, 'GET','/user', 'UserController', null));
            $this->fail();
        } catch (\InvalidArgumentException $e) {}

        $this->assertEquals('Route does not have controllerMethod and therefore cant be added', $e->getMessage());
    }
